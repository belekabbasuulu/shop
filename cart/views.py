...
def cart_add(request, product_id):
    ...
    if form.is_valid():
        ...
        if request.POST.get('overwrite_gty'):
...
def cart_detail(request):
    ...
    for product in products:
        ...
        if product.percent != 0:
            diff += product.difference_price() * cart_item['quantity']
    cart_total_price = sum(
        Decimal(item['price']) * item['quantity'] for item in temp_cart.values())
    return render(request, 'cart/detail.html',
                  {
                      'cart': temp_cart.values(),
                      'cart_total_price': cart_total_price,
                      'difference_total_price': diff
                  }
                  )
